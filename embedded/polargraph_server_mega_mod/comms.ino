...
//  String inS = inString;
  ...
  if (colonPos != -1)
  {
    ...
    {
      Michael_SerialPrint(F("Checksum not matched!:"));
      Michael_SerialPrintln(calcCrc);
...
void comms_parseAndExecuteCommand(String &in)
{
  ...
  {
    Michael_SerialPrintln(F("Command not parsed."));
  }
  in = "";
  commandConfirmed = false;
  comms_ready();
...
void comms_ready()
{
  ...
  Michael_SerialPrintln(READY);
}
void comms_drawing()
{
  Michael_SerialPrintln(DRAWING);
}
void comms_requestResend()
{
  Michael_SerialPrintln(RESEND);
}
void comms_unrecognisedCommand(String &com)
{
  Michael_SerialPrint(F("Sorry, "));
  Michael_SerialPrint(com);
  Michael_SerialPrintln(F(" isn't a command I recognise."));
}  

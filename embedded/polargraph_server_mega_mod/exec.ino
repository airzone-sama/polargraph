...
void exec_changeDrawingDirection() 
{
  ...
  Michael_SerialPrint(F("Changed draw direction mode to be "));
  Michael_SerialPrint(globalDrawDirectionMode);
  Michael_SerialPrint(F(" and direction is "));
...
{
  eeprom_dumpEeprom();
  Michael_SerialPrint(F("PGNAME,"));
  Michael_SerialPrint(machineName);
  Michael_SerialPrintln(CMD_END);
  
  Michael_SerialPrint(F("PGSIZE,"));
  Michael_SerialPrint(machineWidth);
  Michael_SerialPrint(COMMA);
  Michael_SerialPrint(machineHeight);
  Michael_SerialPrintln(CMD_END);

  Michael_SerialPrint(F("PGMMPERREV,"));
  Michael_SerialPrint(mmPerRev);
  Michael_SerialPrintln(CMD_END);

  Michael_SerialPrint(F("PGSTEPSPERREV,"));
  Michael_SerialPrint(motorStepsPerRev);
  Michael_SerialPrintln(CMD_END);
  
  Michael_SerialPrint(F("PGSTEPMULTIPLIER,"));
  Michael_SerialPrint(stepMultiplier);
  Michael_SerialPrintln(CMD_END);
...
void exec_setMotorSpeed(float speed)
{
  ...
  Michael_SerialPrint(F("New max speed: "));
  Michael_SerialPrintln(currentMaxSpeed);
}
...
void exec_setMotorAcceleration(float accel)
{
  ...
  Michael_SerialPrint(F("New acceleration: "));
  Michael_SerialPrintln(currentAcceleration);
}
...
void exec_changePenWidth()
{
  ...
  Michael_SerialPrint(F("Changed Pen width to "));
  Michael_SerialPrint(penWidth);
  Michael_SerialPrint(F("mm"));
...
void exec_changeLengthDirect()
{
  ...
  {
    Michael_SerialPrintln("This point falls outside the area of this machine. Skipping it.");
  }
  else
  {
  Michael_SerialPrintln("dr.5");
...
//      Serial.println(runSpeed);
  ...
  {
    Michael_SerialPrintln("Line is not on the page. Skipping it.");
  }

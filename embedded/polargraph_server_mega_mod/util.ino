...
long getMaxLength()
{
  if (maxLength == 0)
  {
    ...
    Michael_SerialPrint("Calculated maxLength: ");
    Michael_SerialPrintln(maxLength);
  }
  return maxLength;
}
...
void reportPosition()
{
  ...
  {
    Michael_SerialPrint(OUT_CMD_SYNC);
    Michael_SerialPrint(divider(motorA.currentPosition()));
    Michael_SerialPrint(COMMA);
    Michael_SerialPrint(divider(motorB.currentPosition()));
...
void outputAvailableMemory()
{
  ...
  {
    availMem = avMem;
    Michael_SerialPrint(FREE_MEMORY_STRING);
    Michael_SerialPrint(availMem);
    Michael_SerialPrintln(CMD_END);
  }
}
